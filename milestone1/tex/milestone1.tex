...
We need to save the coordinates, and communicate them to the Collector with the RF module.
\item Check for timer $> 5$ minutes (internal)\\
We need to fire an interrupt after a 5 minute time limit and signal saved coordinates to the Collector.
...
We wait for coordinates sent by the Scout, let the RF module send an interrupt.
\item Other robot in specified proximity sensor range (external)\\
Change direction into sensed position.
\end{itemize} 
