...
The Scout robot has 7 distinct tasks as seen below. We chose a strict ordering of priorities depending on the urgency of each task. This will make it easier for the scheduler to choose the next task to be performed. The following table shows urgency in descending order and cost of each task, cost being a rough estimation of required operations.
\begin{center}
\begin{tabular}{| l | | c | c |}
  \hline			
	& Urgency & Cost \\
...
\begin{itemize}
	...
	\item \textbf{approximate cost}: Either generate random coordinates (OR look up next destination for strategy), very low cost. 2
	...
	\item \textbf{dependencies}:dependent on "Orientation and Driving" to be queued up
\end{itemize}
...
\begin{itemize}
	...
	\item \textbf{urgency}:extremely urgent, we want to minimise the time we are using inaccurate coordinates for our postion calculation. Since it is triggered by hardware interrupt we can make it the most urgent task.
